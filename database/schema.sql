...
-- (transport_pay_status was dropped with the ferry's money handling)
...
CREATE TABLE IF NOT EXISTS transport_routes (
  ...
  fare_cents        INTEGER      NOT NULL CHECK (fare_cents > 0),  -- the price of ONE MONTH
  -- The bus runs the same two times every day, so they live on the road
  -- itself. There is no per-day departure table any more: a road that is
  -- running runs every day.
  morning_time      VARCHAR(5)   NOT NULL DEFAULT '06:30',
  evening_time      VARCHAR(5),
  -- The months this road is sold for, 'YYYY-MM'. sell_to NULL = open ended.
  sell_from         VARCHAR(7),
  sell_to           VARCHAR(7),
...
ALTER TABLE transport_routes ADD COLUMN IF NOT EXISTS evening_time VARCHAR(5);
ALTER TABLE transport_routes ADD COLUMN IF NOT EXISTS sell_from    VARCHAR(7);
ALTER TABLE transport_routes ADD COLUMN IF NOT EXISTS sell_to      VARCHAR(7);

-- Per-departure records are gone: a running road runs every day, so there is
-- nothing to store per day. These three tables go with them.
ALTER TABLE ride_bookings DROP COLUMN IF EXISTS trip_id;
DROP TABLE IF EXISTS route_timetables;
DROP TABLE IF EXISTS transport_payments;
DROP TABLE IF EXISTS trips;
DROP TYPE IF EXISTS trip_status;
DROP TYPE IF EXISTS transport_pay_status;
...
DECLARE t TEXT;
BEGIN
  FOREACH t IN ARRAY ARRAY['users', 'food_items', 'orders', 'vehicles', 'driver_profiles', 'transport_routes', 'ride_bookings']
  LOOP
